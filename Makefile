...
CFLAGS	=-g -Wall -O2 -fomit-frame-pointer -fno-builtin
...
tools/system.bin : tools/system.exe 
	copy tools\system.exe system.tmp
	strip system.tmp
	tools/Trans.exe system.tmp tools/system.bin
	del system.tmp
	
Image: boot/bootsect.bin boot/setup.bin $(KERNEL_FILE) tools/build.exe Makefile
#	objcopy -O binary -R .note -R .comment tools/system tools/kernel
	tools\build.exe boot/bootsect.bin boot/setup.bin $(KERNEL_FILE) $(ROOT_DEV)
	@del tools\system.bin
#	rm tools/kernel -f
#	sync
#	del tools\kernel
...
BootImage: boot/bootsect.bin boot/setup.bin tools/build.exe
	tools\build boot/bootsect.bin boot/setup.bin none $(ROOT_DEV)
...
#	(cd lib;make clean)
	@del /S /Q *.a *.o system.map tools\system.exe Image boot\*.bin tools\build.exe
